...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const login = async (email: string, password: string) => {
    ...
    setUser((res.data.userData)); // No need JSON.parse axios converts to object automatically
  ...
  const register = async (name: string, email: string, password: string) => {
    ...
    // setToken(res.data.token);
    // setUser(res.data.userData); // Backend data type fully should match typescript interface, so userId (backend) !== id (frontend)
